...
from evidence_consistency_review_studio import build_review_studio_gate_action_mirror_check
...
def build_report(skill_dir: Path, generated_at: str) -> dict[str, Any]:
    ...
    provider_output = reports["provider_output_evaluation"]
    skill_os2_audit = reports["skill_os2_audit"]
    ...
    archive_path = skill_dir / "dist" / f"{skill_dir.name.removesuffix('-trusted-generation-core-v1')}.zip"
    if not archive_path.exists():
        archive_path = skill_dir / "dist" / "yao-meta-skill.zip"
    actual_archive_sha = hashlib.sha256(archive_path.read_bytes()).hexdigest() if archive_path.is_file() else ""
    archive_hashes = {
        "actual_archive": actual_archive_sha,
        "package_verification": str(package_summary.get("archive_sha256", "")),
        "registry": str(nested(registry, ["package", "checksums", "archive_sha256"], "")),
        "benchmark": str(benchmark_summary.get("archive_sha256", "")),
    }
    archive_hash_ready = bool(actual_archive_sha) and len(set(archive_hashes.values())) == 1
    archive_git_status = git_worktree_status(skill_dir)
    archive_hash_status = "pass" if archive_hash_ready else (
        "warn" if archive_git_status.get("available") and archive_git_status.get("clean") is False else "fail"
    )
    add_check(
        checks,
        key="release-archive-hash-lockstep",
        label="Release archive hash matches package, registry, and benchmark evidence",
        status=archive_hash_status,
        expected={"all_equal_to_actual_archive": actual_archive_sha},
        actual=archive_hashes,
        paths=[
            "dist/yao-meta-skill.zip",
            REQUIRED_REPORTS["package_verification"],
            REQUIRED_REPORTS["registry"],
            REQUIRED_REPORTS["benchmark"],
        ],
        detail=(
            "The release archive is deterministic and all release-facing checksum consumers must name its current SHA256. "
            "A dirty authoring worktree records drift as advisory; a clean release candidate treats drift as a failure."
        ),
    )
    provider_summary = nested(provider_output, ["summary"], {})
    phase1_provider_complete = (
        ...
        and int(provider_summary.get("total_tokens", 250001) or 0) <= 250000
    )
    skill_os2_items = {
        str(item.get("key", "")): item
        for item in skill_os2_audit.get("items", [])
        if isinstance(item, dict)
    }
    provider_item = skill_os2_items.get("provider-holdout", {})
    expected_provider_status = "pass" if phase1_provider_complete else "external_required"
    compare_values(
        checks,
        key="phase1-provider-status-lockstep",
        label="Phase-one provider status is consistent across evaluation, benchmark, and Skill OS audit",
        expected={
            "benchmark_complete": phase1_provider_complete,
            "skill_os2_status": expected_provider_status,
        },
        actual={
            "benchmark_complete": benchmark_summary.get("phase1_provider_matrix_complete"),
            "skill_os2_status": provider_item.get("status"),
        },
        paths=[
            REQUIRED_REPORTS["provider_output_evaluation"],
            REQUIRED_REPORTS["benchmark"],
            REQUIRED_REPORTS["skill_os2_audit"],
        ],
        detail="Legacy model runs remain visible as legacy evidence and cannot mark the fixed 40-call phase-one matrix complete.",
    )
