...
`--publish` checks clean source before provider execution. Provider answer text must remain under `.yao/runs/<run-id>/raw-outputs`; immutable releases may carry hashes, redacted summaries, and relative raw-output locators. Never write `DEEPSEEK_API_KEY` to reports, fixtures, manifests, commands, or logs. Resolve Skill IR through `scripts/skill_ir_paths.py`; wildcard example scanning is forbidden.
