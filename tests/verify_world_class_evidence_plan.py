...
def main() -> None:
    ...
    assert any("evidence-finalize-review" in command for command in tasks["human-adjudication"]["runbook"]), tasks["human-adjudication"]
    assert any("controlled submission ids" in command for command in tasks["human-adjudication"]["runbook"]), tasks["human-adjudication"]
    assert any("summary.reviewer_count == 3" in check for check in tasks["human-adjudication"]["success_checks"]), tasks["human-adjudication"]
    assert any("summary.pair_count == 20" in check for check in tasks["human-adjudication"]["success_checks"]), tasks["human-adjudication"]
    assert "scripts/finalize_provider_review.py" in tasks["human-adjudication"]["evidence_artifacts"], tasks["human-adjudication"]
    assert any("without raw prompts" in item for item in tasks["human-adjudication"]["privacy_contract"]), tasks["human-adjudication"]
    assert any("private answer key remains" in item for item in tasks["human-adjudication"]["privacy_contract"]), tasks["human-adjudication"]
